...
public class ClockGUI extends GuiScreen {
    ...
    Clock mod = ModInstances.getClockMod();
    public ModListView md;
    private ScrollList scrollList;
    private List<String> entries = new ArrayList<>();


    public ClockGUI(ModListView md) {
        this.md = md;
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1001) {
            boolean isEnabled = !mod.is24hr();
            mod.getSettings().hr24 = isEnabled;
            mod.hr24 = isEnabled;
            mod.getSettings().save();
            button.displayString = mod.hr24 ? "Disable" : "Enable";
        } else if (button.id == 0) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRendererObj, "Clock", width / 2, 14, 0xFFFFFF);
    ...
    private class ScrollList extends GuiSlot {
        ...
        public ScrollList() {
            super(ClockGUI.this.mc, ClockGUI.this.width, ClockGUI.this.height, 32, ClockGUI.this.height, 25);
            ClockGUI.this.buttonList.removeIf(b -> b.id >= 1000 && b.id < 1000 + entries.size());
            buttons.clear();
            for (int i = 0; i < entries.size(); i++) {
                String value = "";
                GuiButton button = new GuiButton(i+1000, 0, 0, 75, 20, value);
                buttons.add(button);
                ClockGUI.this.buttonList.add(button);
                switch (i) {
                    case 0:
                        ClockGUI.this.buttonList.get(i).displayString = "";
                        break;
                    case 1:
                        ClockGUI.this.buttonList.get(i).displayString = mod.is24hr() ? "Disable": "Enable";
                        break;
                    case 2:
