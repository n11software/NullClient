...
public class ToggleSprintSneakGUI extends GuiScreen {
    public HUDManager manager;
    ModToggleSprintSneak mod = ModInstances.getToggleSprintSneak();
    public ModListView md;
    private ScrollList scrollList;
    private List<String> entries = new ArrayList<>();


    public ToggleSprintSneakGUI(ModListView md) {
        this.md = md;
    }
    ...
    public void initGui() {
        ...
        entries.clear();
        entries.add("");
        entries.add("Toggle Sneak:");
        entries.add("Toggle Sprint:");
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1001) {
            boolean enabled = !mod.getSettings().sneakToggle;
            mod.getSettings().sneakToggle = enabled;
            mod.isSneakToggle = enabled;
            mod.getSettings().save();
            button.displayString = mod.getSettings().sneakToggle ? "Disable" : "Enable";
        } else if (button.id == 1002) {
            boolean enabled = !mod.getSettings().sprintToggle;
            mod.getSettings().sprintToggle = enabled;
            mod.isSprintToggle = enabled;
            mod.getSettings().save();
            button.displayString = mod.getSettings().sprintToggle ? "Disable" : "Enable";
        } else if (button.id == 0) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRendererObj, "Toggle Sprint/Sneak", width / 2, 14, 0xFFFFFF);
    ...
    private class ScrollList extends GuiSlot {
        ...
        public ScrollList() {
            super(ToggleSprintSneakGUI.this.mc, ToggleSprintSneakGUI.this.width, ToggleSprintSneakGUI.this.height, 32, ToggleSprintSneakGUI.this.height, 25);
            ToggleSprintSneakGUI.this.buttonList.removeIf(b -> b.id >= 1000 && b.id < 1000 + entries.size());
            buttons.clear();
            for (int i = 0; i < entries.size(); i++) {
                String value = "";
                GuiButton button = new GuiButton(i+1000, 0, 0, 75, 20, value);
                buttons.add(button);
                ToggleSprintSneakGUI.this.buttonList.add(button);
                switch (i) {
                    case 0:
                        ToggleSprintSneakGUI.this.buttonList.get(i).displayString = "";
                        break;
                    case 1:
                        ToggleSprintSneakGUI.this.buttonList.get(i).displayString = mod.getSettings().sneakToggle ? "Disable": "Enable";
                        break;
                    case 2:
                        ToggleSprintSneakGUI.this.buttonList.get(i).displayString = mod.getSettings().sprintToggle ? "Disable": "Enable";
                        break;
                    case 3:
