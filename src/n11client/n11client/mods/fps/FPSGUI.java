...
public class FPSGUI extends GuiScreen {
    ...
    ModFPS mod = ModInstances.getFPS();
    public ModListView md;
    private ScrollList scrollList;
    private List<String> entries = new ArrayList<>();


    public FPSGUI(ModListView md) {
        this.md = md;
    }
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRendererObj, "FPS", width / 2, 14, 0xFFFFFF);
    ...
    private class ScrollList extends GuiSlot {
        ...
        public ScrollList() {
            super(FPSGUI.this.mc, FPSGUI.this.width, FPSGUI.this.height, 32, FPSGUI.this.height, 25);
            FPSGUI.this.buttonList.removeIf(b -> b.id >= 1000 && b.id < 1000 + entries.size());
            buttons.clear();
            for (int i = 0; i < entries.size(); i++) {
                String value = "";
                GuiButton button = new GuiButton(i+1000, 0, 0, 75, 20, value);
                buttons.add(button);
                FPSGUI.this.buttonList.add(button);
                switch (i) {
                    case 0:
                        FPSGUI.this.buttonList.get(i).displayString = "";
                        break;
                    case 1:
