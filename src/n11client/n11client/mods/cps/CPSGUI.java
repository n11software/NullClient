...
public class CPSGUI extends GuiScreen {
    ...
    CPS mod = ModInstances.getCPSMod();
    public ModListView md;
    private ScrollList scrollList;
    private List<String> entries = new ArrayList<>();


    public CPSGUI(ModListView md) {
        this.md = md;
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 1001) {
            mod.getSettings().left = !mod.getSettings().left;
            mod.getSettings().save();
            button.displayString = mod.getSettings().left ? "Disable" : "Enable";
        } else if (button.id == 1002) {
            mod.getSettings().right = !mod.getSettings().right;
            mod.getSettings().save();
            button.displayString = mod.getSettings().right ? "Disable" : "Enable";
        } else if (button.id == 0) {
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRendererObj, "CPS", width / 2, 14, 0xFFFFFF);
    ...
    private class ScrollList extends GuiSlot {
        ...
        public ScrollList() {
            super(CPSGUI.this.mc, CPSGUI.this.width, CPSGUI.this.height, 32, CPSGUI.this.height, 25);
            CPSGUI.this.buttonList.removeIf(b -> b.id >= 1000 && b.id < 1000 + entries.size());
            buttons.clear();
            for (int i = 0; i < entries.size(); i++) {
                String value = "";
                GuiButton button = new GuiButton(i+1000, 0, 0, 75, 20, value);
                buttons.add(button);
                CPSGUI.this.buttonList.add(button);
                switch (i) {
                    case 0:
                        CPSGUI.this.buttonList.get(i).displayString = "";
                        break;
                    case 1:
                        CPSGUI.this.buttonList.get(i).displayString = mod.getSettings().left ? "Disable": "Enable";
                        break;
                    case 2:
                        CPSGUI.this.buttonList.get(i).displayString = mod.getSettings().right ? "Disable": "Enable";
                        break;
                    case 3:
