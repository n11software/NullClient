...
public class PingGUI extends GuiScreen {
    public HUDManager manager;
    ModPing mod = ModInstances.getPing();
    public ModListView md;
    private ScrollList scrollList;
    private List<String> entries = new ArrayList<>();


    public PingGUI(ModListView md) {
        this.md = md;
    }
    ...
    protected void actionPerformed(GuiButton button) throws IOException {
        if (button.id == 0) {
            ...
            mod.getSettings().enabled = !mod.isEnabled();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        drawCenteredString(mc.fontRendererObj, "Ping", width / 2, 14, 0xFFFFFF);
    ...
    private class ScrollList extends GuiSlot {
        ...
        public ScrollList() {
            super(PingGUI.this.mc, PingGUI.this.width, PingGUI.this.height, 32, PingGUI.this.height, 25);
            PingGUI.this.buttonList.removeIf(b -> b.id >= 1000 && b.id < 1000 + entries.size());
            buttons.clear();
            for (int i = 0; i < entries.size(); i++) {
                String value = "";
                GuiButton button = new GuiButton(i+1000, 0, 0, 75, 20, value);
                buttons.add(button);
                PingGUI.this.buttonList.add(button);
                switch (i) {
                    case 0:
                        PingGUI.this.buttonList.get(i).displayString = "";
                        break;
                    case 1:
